...
<form>
	...
	<button type="button" class="but_add" onclick="javascript:_editIndicatorSta();"><i class="iconfont icon-xinzeng"></i>新增</button>	
										...
										<c:if test="${not empty pageBean.items }">
											<c:forEach items="${pageBean.items}" var="item"
												...
												<tr>
													...
													<input type="button" class="table_button" onclick="javascript:_editIndicatorSta('${item.id}');" value="修改">
													<input type="button" class="table_button" onclick="javascript:_deleteIndicatorSta('${item.id}');" value="删除">
													</td>
												</tr>
											</c:forEach>
										</c:if>
...
<%-- 	<jsp:include page="/jsp/cq/includeJs.jsp" /> --%>
	<script type="text/javascript">
		...
		// 删除产权信息
		function _deleteIndicatorSta(id){
			var params = {
					"id" : id
				};
				parent.win
						.confirm(
								...
								function(r) {
									if (r) {
										var url = '${pageContext.request.contextPath}/indicatorsta/_delete';
										$.ajax({
											type : 'post',
											url : url,
											dateType : 'json',
		...
		function _editIndicatorSta(id) {
			if(id == '' || id == undefined){
				var url = "${pageContext.request.contextPath}/indicatorsta/_edit";
			}else{
				var url = "${pageContext.request.contextPath}/indicatorsta/_edit?id="+id;
			}
		...
		function _upload() {	
			var url = "${pageContext.request.contextPath}/indicatorsta/_upload";			
		...
		function _downloadFile() {
			var url="static/temp/dataSta/数据指标导入模板.xlsx";
			var name="数据指标导入模板.xlsx";
			console.log(url);
