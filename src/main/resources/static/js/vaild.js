! function(W) {
	...
	var add_tip = W.vaild.add_tip = function(el,msgs){
		var t = $('<div class="v-confirm-tip">'+msgs.map(function(e){
			return '<div class="tip-item"'+(e.vaild?'ok':'')+'><span no>×</span><span ok>√</span><span>'+e.msg+'</span></div>'
		}).join('')+'</div>').css({
	...
	W.vaild.all = function(el){
		return $('input[check],textarea[check],select[check]').get().filter(function(el){
	...
	$(function() {
		W.vaild.init()
		setTimeout(function(){
			$('input[check],textarea[check],select[check]').on('input click', function() {
			...
			$('body').on('input click', 'input[check],textarea[check],select[check]', function() {
				if(this.getAttribute('self-bind',true)==='true') return;
				// tip
				$('.v-confirm-tip').remove()
				W.vaild.tip(this)
			}).on('focus', 'input[check],textarea[check],select[check]', function() {
				if(this.getAttribute('self-bind',true)=='true') return;
				// valid
				// $(this).removeClass('error')
			}).on('blur', 'input[check],textarea[check],select[check]', function() {
				if(this.getAttribute('self-bind',true)==='true') return;
				// tip
				$('.v-confirm-tip').remove()
				// vaild
